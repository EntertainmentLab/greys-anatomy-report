...
export function usePolicyByLeaningData() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch(`${import.meta.env.BASE_URL}policy-support-by-political-orientation.json`)
        ...
        const processedData = policyData.map(d => ({
          ...
          n: deepUnwrap(d.n),
          pol_orientation: deepUnwrap(d.pol_orientation)
        }))
